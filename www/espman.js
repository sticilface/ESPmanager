function getBaseUrl() {
    ...
    return "http://" + window.location.hostname + "/espman/"; 
   // var re = new RegExp(/^.*\//);
   // return re.exec(window.location.hostname) + "/espman/";
}
...
$(document).on("pagecreate", "#wifipage", function() {
    ...
    $("#apply_sta").click(function() {
        $.post(_home_device + "data.esp", $(this.form).serialize());
...
function GetAboutVars() {
    $.post(_home_device + "data.esp", "AboutPage", function(results) {
            ...
            $("#aboutvars").append("<br>FS Size = " + results.FS.totalBytes);
            $("#aboutvars").append("<br>FS Used = " + results.FS.usedBytes);
