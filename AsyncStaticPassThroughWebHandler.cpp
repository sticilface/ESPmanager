...
bool AsyncStaticPassThroughWebHandler::canHandle(AsyncWebServerRequest *request)
{
	...
	if (request->url().startsWith(_uri)) {
		...
		if (_fs.exists(path) || _fs.exists(path + ".gz")) {
			DEBUGF("[AsyncStaticPassThroughWebHandler::canHandle] TRUE\n");
			return true;
		}
	}
...
String AsyncStaticPassThroughWebHandler::_getPath(AsyncWebServerRequest *request)
{

	String path = request->url();
	DEBUGF("[AsyncStaticPassThroughWebHandler::_getPath]\n");
	...
	DEBUGF(" final path = %s\n", path.c_str());
	DEBUGF("[AsyncStaticPassThroughWebHandler::_getPath] END\n\n");

	return path;
}


void AsyncStaticPassThroughWebHandler::handleRequest(AsyncWebServerRequest *request)
{
